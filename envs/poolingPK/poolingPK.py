...
# white-背景 Player-用户编写的智能体-黄色 Sniper-拐角扫描攻击型-蓝色 Rook-水平移动-红色，四方向扫描 Counter-原地扫描攻击范围内攻击，非攻击范围朝目标方向移动-绿色
COLOR_MAP = {"white": (255, 255, 255),
             "Player": (255, 128, 0),
             "Sniper": (0, 0, 255),
             "Rook": (255, 20, 20),
...
class PoolingPK(PyGameWrapper):
    ...
    def __init__(self, draw=True, width=300, height=300, num_sniper=1, num_player=1, num_rook=1, num_counter=1,
                 full_blood=1000):
        # key_mapping作用是什么
        ...
        self.PLAYER_SPEED = 50  # 玩家速度(0,100)整数
        ...
        self.MAX_SNIPER_NUM = num_sniper  # !!!
        self.SNIPER_COLOR = COLOR_MAP['Sniper']
        self.SNIPER_SPEED = 50
    ...
    def _rand_postion(self, agents):
        pos = []
        for agent in agents:
            pos_x = random.uniform(agent.radius,
                                   self.width - agent.radius)  # 从一个均匀分布[low,high)中随机采样，注意定义域是左闭右开，即包含low，不包含high
    ...
    def init(self):
        ...
        for _ in range(self.COUNTER_NUM):
            counter = Counter(id, self.COUNTER_RADIUS, self.COUNTER_COLOR,
                              self.COUNTER_SPEED, self.width, self.height, self.COUNTER_BLOOD, self.COUNTER_ANGLE,
                              type='Counter')
            ...
            id += 1
        for _ in range(self.SNIPER_NUM):
    ...
    def step(self, dt):
        ...
        for rook in self.rooks:
            for counter in self.counters:
                ...
                if (count_distance_fast(rook.pos[0], rook.pos[1], counter.pos[0], counter.pos[1]) <= 100) and (
                        count_distance_fast(rook.pos[0], rook.pos[1], counter.pos[0], counter.pos[1]) > 20) and (
                        abs(counter.pos[1] - rook.pos[1]) <= 2 or abs(counter.pos[0] - rook.pos[0]) <= 2):
                ...
                if count_distance_fast(rook.pos[0], rook.pos[1], counter.pos[0], counter.pos[1]) <= 100:
                    self.ROOK_BLOOD -= 10
                    dis = count_distance_fast(rook.pos[0], rook.pos[1], counter.pos[0], counter.pos[1])
                    self.draw_line(rook.rect.center, math.sin(
                        cal_angle((rook.pos[0], rook.pos[1]), (counter.pos[0], counter.pos[1]),
                                  (counter.pos[0], rook.pos[1]))), \
                                   math.cos(cal_angle((rook.pos[0], rook.pos[1]), (counter.pos[0], counter.pos[1]),
                                                      (counter.pos[0], rook.pos[1]))), \
                                   counter, dis, COLOR_MAP["Counter"])
                    # self.reward[player.id] += self.rewards["positive"]
                '''
                if self.PLAYER_BLOOD <= 0:
                    self.reward[player.id] += self.rewards["loss"]
                    self.players.remove(player)
                    self.PLAYER_NUM = 0
                '''
                if self.COUNTER_BLOOD <= 0:
                    # self.reward[counter.id] += self.rewards["win"]
                    self.counters.remove(counter)
                    self.COUNTER_NUM = 0
                if self.ROOK_BLOOD <= 0 and self.ROOK_NUM > 0:
                    # self.reward[counter.id] += self.rewards["win"]
                    self.rooks.remove(rook)
                    self.ROOK_NUM = 0

            for sniper in self.snipers:
                if (count_distance_fast(rook.pos[0], rook.pos[1], sniper.pos[0], sniper.pos[1]) <= 100) and (
                        count_distance_fast(rook.pos[0], rook.pos[1], sniper.pos[0], sniper.pos[1]) > 20):
                    self.ROOK_BLOOD -= 10
                    self.SNIPER_BLOOD -= 10
                    dis = count_distance_fast(rook.pos[0], rook.pos[1], sniper.pos[0], sniper.pos[1])
                    self.draw_line(rook.rect.center, 1, 0, rook, dis, COLOR_MAP["Sniper"])
                    self.draw_line(rook.rect.center, 0, 1, rook, dis, COLOR_MAP["Sniper"])
                    self.draw_line(rook.rect.center, -1, 0, rook, dis, COLOR_MAP["Sniper"])
                    self.draw_line(rook.rect.center, 0, -1, rook, dis, COLOR_MAP["Sniper"])
                    # self.reward[player.id] += self.rewards["positive"]
                elif count_distance_fast(rook.pos[0], rook.pos[1], sniper.pos[0], sniper.pos[1]) <= 20:
                    self.SNIPER_BLOOD -= (self.SNIPER_SPEED - self.ROOK_SPEED) * 0.03
                    self.ROOK_BLOOD -= (self.SNIPER_SPEED - self.ROOK_SPEED) * 0.03

                if count_distance_fast(rook.pos[0], rook.pos[1], sniper.pos[0], sniper.pos[1]) <= 50:   #50?100?
                    self.ROOK_BLOOD -= 10    #!!!hua gong ji xian!!!
                    dis = count_distance_fast(rook.pos[0], rook.pos[1], sniper.pos[0], sniper.pos[1])
                    self.draw_line(rook.rect.center, math.sin(
                        cal_angle((rook.pos[0], rook.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                  (sniper.pos[0], rook.pos[1]))), \
                                   math.cos(cal_angle((rook.pos[0], rook.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                                      (sniper.pos[0], rook.pos[1]))), \
                                   sniper, dis, COLOR_MAP["Sniper"])
                if self.SNIPER_BLOOD <= 0:
                    # self.reward[counter.id] += self.rewards["win"]
                    self.snipers.remove(sniper)
                    self.SNIPER_NUM = 0
            ...
            for player in self.players:
                if count_distance_fast(rook.pos[0], rook.pos[1], player.pos[0], player.pos[1]) <= 100 and (
                        count_distance_fast(rook.pos[0], rook.pos[1], player.pos[0], player.pos[1]) > 20) and (
                        abs(player.pos[1] - rook.pos[1]) <= 2 or abs(player.pos[0] - rook.pos[0]) <= 2):
                    ...
                    self.reward[player.id] += self.rewards["negative"]
                if count_distance_fast(rook.pos[0], rook.pos[1], player.pos[0], player.pos[1]) <= 50:   #50?100?
                    self.ROOK_BLOOD -= 10
                    dis = count_distance_fast(rook.pos[0], rook.pos[1], player.pos[0], player.pos[1])
                    self.draw_line(rook.rect.center, math.sin(
                        cal_angle((rook.pos[0], rook.pos[1]), (player.pos[0], player.pos[1]),
                                  (player.pos[0], rook.pos[1]))), \
                                   math.cos(cal_angle((rook.pos[0], rook.pos[1]), (player.pos[0], player.pos[1]),
                                                      (player.pos[0], rook.pos[1]))), \
                                   player, dis, COLOR_MAP["Player"])
                    self.reward[player.id] += self.rewards["positive"]
                if self.ROOK_BLOOD <= 0 and self.ROOK_NUM > 0:
        ...
        for player in self.players:
            # player_pair = []
            for counter in self.counters:
                if (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) <= 100) and (
                        count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) >= 20):
                    self.COUNTER_BLOOD -= 10
                    self.PLAYER_BLOOD -= 10
                    dis = count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1])
                    self.draw_line(player.rect.center, math.sin(
                        cal_angle((player.pos[0], player.pos[1]), (counter.pos[0], counter.pos[1]),
                                  (counter.pos[0], player.pos[1]))), \
                                   math.cos(cal_angle((player.pos[0], player.pos[1]), (counter.pos[0], counter.pos[1]),
                                                      (counter.pos[0], player.pos[1]))), \
                                   counter, dis, COLOR_MAP["Counter"])
                    self.draw_line(counter.rect.center, math.cos(cal_angle((player.pos[0], player.pos[1]), (counter.pos[0], counter.pos[1]),
                                                                           (counter.pos[0], player.pos[1]))), \
                                   math.sin(cal_angle((player.pos[0], player.pos[1]), (counter.pos[0], counter.pos[1]), (counter.pos[0], player.pos[1]))), \
                                   player, dis, COLOR_MAP["Player"])
                    # self.draw_line(center, sin_angle, cos_angle, player, dis_to_wall, COLOR_MAP["Counter"])
                    self.reward[player.id] += self.rewards["negative"]
                # elif count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) > 150 and (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) <= 90):
                # self.reward[player.id] += self.rewards["positive"]
                elif count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) <= 20:
                ...
                if self.PLAYER_BLOOD <= 0:
                    ...
                    self.players.remove(player)
                    self.PLAYER_NUM = 0
                if self.COUNTER_BLOOD <= 0 and self.COUNTER_NUM > 0:
                    self.reward[player.id] += self.rewards["win"]
                    self.counters.remove(counter)
                    self.COUNTER_NUM = 0
            ...
            '''
            for rook in self.rooks:
                if (count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) <= 100) and (count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) > 20):
                    self.ROOK_BLOOD -= 10
                    self.PLAYER_BLOOD -= 10
                    self.reward[player.id] += self.rewards["negative"]
                    dis = count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1],)
                    self.draw_line(player.rect.center, math.sin(cal_angle((player.pos[0], player.pos[1]),(rook.pos[0],rook.pos[1]),(rook.pos[0],player.pos[1]))), \
                                   math.cos(cal_angle((player.pos[0], player.pos[1]),(rook.pos[0],rook.pos[1]),(rook.pos[0],player.pos[1]))), \
                                   rook, dis, COLOR_MAP["Rook"])
            ...
            for sniper in self.snipers:
                if (count_distance_fast(player.pos[0], player.pos[1], sniper.pos[0], sniper.pos[1]) <= 100) and (
                        count_distance_fast(player.pos[0], player.pos[1], sniper.pos[0], sniper.pos[1]) > 20):
                    self.SNIPER_BLOOD -= 10
                    self.PLAYER_BLOOD -= 10
                    self.reward[player.id] += self.rewards["negative"]
                    dis = count_distance_fast(player.pos[0], player.pos[1], sniper.pos[0], sniper.pos[1] )
                    self.draw_line(player.rect.center, math.sin(
                        cal_angle((player.pos[0], player.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                  (sniper.pos[0], player.pos[1]))), \
                                   math.cos(cal_angle((player.pos[0], player.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                                      (sniper.pos[0], player.pos[1]))), \
                                   sniper, dis, COLOR_MAP["Sniper"])
                    self.draw_line(sniper.rect.center, math.cos(
                        cal_angle((player.pos[0], player.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                  (sniper.pos[0], player.pos[1]))), \
                                   math.sin(cal_angle((player.pos[0], player.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                                      (sniper.pos[0], player.pos[1]))), \
                                   player, dis, COLOR_MAP["Player"])
                elif count_distance_fast(player.pos[0], player.pos[1], sniper.pos[0], sniper.pos[1]) <= 20:
                    self.PLAYER_BLOOD -= (self.PLAYER_SPEED - self.SNIPER_BLOOD) * 0.03
                    self.SNIPER_BLOOD -= (self.PLAYER_SPEED - self.SNIPER_NUM) * 0.03
                    self.reward[player.id] += self.rewards["negative"]
                if self.SNIPER_NUM <= 0 and self.SNIPER_BLOOD > 0:
                    self.reward[player.id] += self.rewards["win"]
                    self.snipers.remove(sniper)
                    self.SNIPER_NUM = 0
                if self.PLAYER_BLOOD <= 0:
        ...
        for counter in self.counters:
            # player_pair = []
            for sniper in self.snipers:
                if (count_distance_fast(counter.pos[0], counter.pos[1], sniper.pos[0], sniper.pos[1]) <= 100) and (
                        count_distance_fast(counter.pos[0], counter.pos[1], sniper.pos[0], sniper.pos[1]) >= 20):
                    self.SNIPER_BLOOD -= 5
                    self.COUNTER_BLOOD -= 5
                    dis = count_distance_fast(counter.pos[0], counter.pos[1], sniper.pos[0], sniper.pos[1])
                    self.draw_line(counter.rect.center, math.sin(
                        cal_angle((counter.pos[0], counter.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                  (sniper.pos[0], counter.pos[1]))), \
                                   math.cos(cal_angle((counter.pos[0], counter.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                                      (sniper.pos[0], counter.pos[1]))), \
                                   sniper, dis, COLOR_MAP["Sniper"])
                    self.draw_line(sniper.rect.center, math.cos(
                        cal_angle((counter.pos[0], counter.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                  (sniper.pos[0], counter.pos[1]))), \
                                   math.sin(cal_angle((counter.pos[0], counter.pos[1]), (sniper.pos[0], sniper.pos[1]),
                                                      (sniper.pos[0], counter.pos[1]))), \
                                   counter, dis, COLOR_MAP["Counter"])
                    # self.draw_line(center, sin_angle, cos_angle, player, dis_to_wall, COLOR_MAP["Counter"])
                # elif count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) > 150 and (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) <= 90):
                # self.reward[player.id] += self.rewards["positive"]
                elif count_distance_fast(counter.pos[0], counter.pos[1], sniper.pos[0], sniper.pos[1]) <= 20:
                    self.COUNTER_BLOOD -= abs(self.COUNTER_SPEED - self.SNIPER_SPEED) * 0.03
                    self.SNIPER_BLOOD -= abs(self.COUNTER_SPEED - self.SNIPER_SPEED) * 0.03
                    # self.reward[player.id] += self.rewards["negative"]
                ...
                if self.COUNTER_BLOOD <= 0 and self.COUNTER_NUM > 0:
                    ...
                    self.counters.remove(counter)
                    self.COUNTER_NUM = 0
                if self.SNIPER_BLOOD <= 0 and self.SNIPER_NUM > 0:
                    # self.reward[player.id] += self.rewards["positive"]
                    self.snipers.remove(sniper)
                    self.SNIPER_NUM = 0
...
if __name__ == "__main__":
    ...
    game = PoolingPK(width=300, height=300, num_sniper=1, num_player=1, num_rook=1, num_counter=1, full_blood=1000,
                     draw=True)
