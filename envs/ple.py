...
class PLE(object):
    def __init__(self,
                 ...
                 add_noop_action=True, rng=24, max_episodes=100,
                 resized_rows=84,
                 resized_cols=84):
    ...
    def game_over(self):
        return self.episode_step >= self.max_episodes or self.game.game_over()
    ...
    def act(self, actions):
        ...
        info = np.zeros(self.game.info.shape, dtype=int)
        for i in range(self.num_steps):
            time_elapsed = self._tick()
            step_reward, step_info = self.game.step(time_elapsed)
            info += step_info[:]
            reward += step_reward[:]
            self._draw_frame()
        ...
        return ob, reward, terminal_flag, info

    def _tick(self):
        if self.force_fps:
            return 1000 / self.fps if self.fps == 1 else 1000.0 / self.fps
        else:
